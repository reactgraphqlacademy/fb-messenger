...
- [ ] 1. In `src/components/Layout/TopBar.js` create a MyLink component that returns a `Link` component if the prop `to` doesn't startWith "http" (you can use `props.to.startsWith("http")`), otherwise it should return `a`. Replace your Link components in `src/components/Layout/TopBar.js` with your `MyLink` component. Heads up! you need to think of the `children` prop.
